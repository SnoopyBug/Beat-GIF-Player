...
def pil_frame_to_pixmap(img: Image.Image, target_size: Optional[Tuple[int, int]] = None, scale: float = 1.0) -> QPixmap:
...
def load_gif_frames(gif_path: str, target_size: Optional[Tuple[int, int]] = None, scale: float = 1.0) -> Tuple[List[QPixmap], List[int]]:
...
class GifWindow(QWidget):
    def __init__(self, cfg: AppConfig):
        ...
        self.frames, self.original_durations_ms = load_gif_frames(
            cfg.gif_path,
            target_size=(self.window_width, self.window_height),
            scale=cfg.window_scale,
        )
        self.frame_index = 0
